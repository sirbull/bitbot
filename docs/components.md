...
| 11 | Setup button | Built-in BOOT button (GPIO0) | — | GPIO | Confirmed, part of XIAO |

## Not used

- The SD card reader on the Sense board (left empty; shares pins with the display SPI).
- Motors, motor drivers and servos.
- Battery voltage sensing (no divider on GPIO6). The module's 4-LED gauge is
  the only charge indicator.
