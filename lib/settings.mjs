...
export function validateSettings(patch, current = defaults()) {
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    if (['string', 'url', 'enum'].includes(rule.type)) {
      ...
      if (rule.format === 'language') assert(['auto', 'auto-all'].includes(value) || /^[a-z]{2,8}(?:-[a-z0-9]{1,8})*$/iu.test(value), 'Use a valid BCP-47 language code.');
  ...
  const speechProvider = result.speechProvider === 'same' ? result.provider : result.speechProvider;
  assert(speechProvider === 'local' || result.pitch === 0, 'Numerical pitch is only reserved for the self-hosted speech adapter.');
  assert(result.provider === 'local' || !/^(http|ws):/u.test(result.endpoint), 'Cloud service URLs must use HTTPS or WSS.');
  assert(speechProvider === 'local' || !/^(http|ws):/u.test(result.speechEndpoint), 'Cloud speech URLs must use HTTPS or WSS.');
  return result;
}
...
export function updateDocument(document, input) {
  assert(object(input) && Object.keys(input).every(key => ['settings', 'apiKey', 'clearApiKey', 'speechApiKey', 'clearSpeechApiKey'].includes(key)), 'Invalid settings request.');
  ...
  assert(input.speechApiKey === undefined || (typeof input.speechApiKey === 'string' && bytes(input.speechApiKey) <= 256 && !/[\u0000-\u0020\u007f]/u.test(input.speechApiKey)), 'Invalid speech API key.');
  assert(input.clearApiKey === undefined || typeof input.clearApiKey === 'boolean', 'Invalid key removal request.');
  assert(input.clearSpeechApiKey === undefined || typeof input.clearSpeechApiKey === 'boolean', 'Invalid speech key removal request.');
  assert(!(input.apiKey && input.clearApiKey), 'Choose either replacement or removal of the key.');
  assert(!(input.speechApiKey && input.clearSpeechApiKey), 'Choose either replacement or removal of the speech key.');
  const speechProvider = settings.speechProvider === 'same' ? settings.provider : settings.speechProvider;
  assert(speechProvider !== settings.provider || (!input.speechApiKey && !input.clearSpeechApiKey), 'The selected AI and speech services share the API key entered above.');
  ...
  if (input.clearSpeechApiKey) delete keys[speechProvider];
  else if (input.speechApiKey) keys[speechProvider] = input.speechApiKey;
  return { ...document, settings, keys };
}

export function publicSettings(document) {
  const speechProvider = document.settings.speechProvider === 'same' ? document.settings.provider : document.settings.speechProvider;
  return { settings: document.settings, hasApiKey: Boolean(document.keys[document.settings.provider]), hasSpeechApiKey: Boolean(document.keys[speechProvider]), keyProviders: Object.keys(document.keys) };
}
